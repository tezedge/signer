...
class KeysResource(object):
    ...
    def parse_message(self, msg_bytes):
        ...
        operation_tag = msg_bytes[33]
        operation = None

        # logging.info("Message with watermark: {}".format(watermark))
    ...
    def parse_block(self, msg_bytes):
        # the field fitness has a variable length, we need get that first
        ...
        try:
            ...
            if self._decode_bool(msg_bytes[self.BLOCK_PRESENCE_OF_NONCE_INDEX_WITHOUT_FITNESS + bytes_in_fitness]):
            ...
            # if the seed_nonce_hash field is present add it to the dictionary, its
    ...
    def parse_delegation_with_reveal(self, msg_bytes):
        delegatio_with_reveal_msg = None
        try:
